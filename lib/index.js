...
if (!process.env.VERCEL_API_ENABLED) {
  var ses = new AWS.SES();
  AWS.config.region = process.env.AWS_REGION;
} else {
  // https://vercel.com/docs/concepts/projects/environment-variables#reserved-environment-variables
  // Some variables are not compatible with Vercel, so we use this option
  var {
    AMAZON_ACCESS_KEY: accessKeyId,
    AMAZON_SECRET_ACCESS_KEY: secretAccessKey,
    AMAZON_REGION: region,
  } = process.env;
  var config = { accessKeyId, secretAccessKey, region };
